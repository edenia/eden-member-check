...
summary: The intent of the `{{ checkmember }}` action is to check if  the `account` is a member account.
---)";


    // CLAUSES
    const char* datastorage_clause = R"(The edenmember application values the security of personal data. We may process only minimal user data as much as it is necessary to maintain the edenmember application running. We only receive and store any information you knowingly provide to us when you create an account, or fill using the app.)";

    const char* datausage_clause = R"(This smart contract will process user data to keep the edenmember application running. Any of the information we collect from you may be used for these purposes:
